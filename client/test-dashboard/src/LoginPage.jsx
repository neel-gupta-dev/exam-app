    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <main className="grid w-full max-w-6xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm lg:grid-cols-[1fr_440px]">
        <section className="hidden bg-slate-950 p-10 text-white lg:flex lg:flex-col lg:justify-between">
          <div>
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-sm font-black text-slate-950">V</div>
              <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={top} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="text-lg font-black">{top}</p>
                <p className="text-xs font-bold uppercase tracking-widest text-slate-400">{bottom}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="p-6 sm:p-10">
          ...
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-slate-400">Student Login</p>
          <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-950">Welcome back</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">Sign in to continue your tests and analytics.</p>
          ...
          <div className="mt-8 rounded-2xl bg-slate-100 p-1">
            <button
              ...
              className={`w-full rounded-xl px-4 py-2.5 text-sm font-black transition ${
                activeTab === 'scholar' ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500'
              }`}
            >
              Scholar Login
            </button>
          ...
          {activeTab === 'scholar' && (
            <form onSubmit={handleScholarLogin} className="mt-6 space-y-5">
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-widest text-slate-500">Email Address</label>
              ...
              <div>
                <label className="mb-2 block text-xs font-black uppercase tracking-widest text-slate-500">Password</label>
                <div className="relative">
